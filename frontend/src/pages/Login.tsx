...
const Login = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-transparent relative flex items-center justify-center overflow-hidden font-sans">

      {/* Background Split Canvas with Wave */}
      <div className="absolute top-0 left-0 w-full h-[45%] bg-gradient-to-r from-[#1D61D1] to-[#4F46E5]">
        <svg
      ...
      {/* Back Button to Detect Scam */}
      <button
        onClick={handleBackToDetect}
        className="fixed top-6 left-6 z-20 flex items-center gap-2 bg-white/90 backdrop-blur-sm hover:bg-white px-4 py-2 rounded-full shadow-md border border-gray-200 transition-all duration-200 group"
      >
        <ArrowLeft className="w-4 h-4 text-[#1D61D1] group-hover:-translate-x-0.5 transition-transform" />
        <span className="text-sm font-medium text-gray-700">Try Detect Scam</span>
      </button>
      ...
      <div className="relative z-10 bg-white w-full max-w-[420px] rounded-[24px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] p-10 mx-4 border border-gray-100">
        ...
        <div className="space-y-4">
          ...
          <button className="w-full bg-gradient-to-r from-[#1D61D1] to-[#4F46E5] hover:from-[#1557b8] hover:to-[#4338ca] text-white font-semibold py-3.5 px-4 rounded-[12px] transition-all duration-200 shadow-md shadow-blue-500/20">
            Login with Email
          </button>
        ...
        <p className="text-center text-xs text-gray-400 mt-6">
          Don't have an account? Sign up above to save your history
        </p>
      </div>
    </div>
  );
